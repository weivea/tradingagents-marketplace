...
| `append_discussion` | date, speaker, markdown, next_speaker, reason | 讨论纪要 append 一段发言；`next_speaker` ∈ {aggressive, neutral, conservative, end}，作为 HTML 注释嵌入供编排器解析 |
...
4. 调用 `append_discussion(today, style, markdown, next_speaker, reason)` 追加一段，并在同一次调用里声明"下一个发言者"（见 Component 3 的对话机制）

**强制输出契约：** 每次发言 append 时必须声明 `next_speaker ∈ {aggressive, neutral, conservative, end}`：

- 正常情况：点名希望回应自己的人（或认为此刻该听谁说）
- 认为话题已谈透：选 `end`
- `reason` 是 1 句话解释为什么点这位（会存入 discussion 文件作为隐藏 HTML 注释，正文不可见）
...
### 对话机制：Agent 自主传话（Self-Directed Turn-Taking）

三位 agent **不并发、不广播**，而是通过 **共享 markdown 文件 + agent 自主点名下一位** 模拟自由对话。Claude Code 本身无并发 runtime，每次唤起一位 agent 都是独立的短命子进程，其"记忆"仅来自 prompt + 能读到的文件。

**流程：**

1. `get_pnl` × 3 获取三人今日战绩
2. `init_discussion(today, pnl_summary)` 建文件、写 header
3. **开场** — 随机选一位 agent（或固定从激进开始）唤起其 `/join-discussion`
4. **循环（自由对话）**：
   - 读 discussion 文件末尾，解析上一位发言者的 `next_speaker`
   - 根据值决定下一步：
     - `aggressive` / `neutral` / `conservative` → 唤起对应 agent 的 `/join-discussion`
     - `end` → 记一次"结束票"
   - 结束条件（满足任一）：
     - **连续 2 位不同 agent 投 `end`** → 自然结束
     - **总轮数 ≥ 50** → 强制结束（硬上限）
5. **收官总结** — 无论如何结束，最后强制唤起 `trader-neutral` 写一段当日总结

**编排器护栏：**

- 同一 agent **连续被点名 3 次** → 编排器强制切换到另一位（防抢麦 / 双人对打把第三位晾着）
- agent 忘记输出 `next_speaker` → 编排器按"被提及但未发言"或随机规则选下一位
- `next_speaker` 指向自己 → 视为无效，按上一条规则处理
- 每位 agent 至少被唤起 1 次才允许 `end` 生效（防第 1 位直接投 end）

**Agent prompt 里的强制契约示例：**

```markdown
发言完成后，你必须以如下 YAML 格式结尾（会作为 HTML 注释存入文件，读者看不见）：

<!--
next_speaker: conservative
reason: 我质疑了保守的低仓位策略，想听他反驳
-->
```

编排器用正则 `<!--\s*next_speaker:\s*(\w+)` 从文件末尾解析。
...
**激进选手：** 兄弟们我今天追了龙虎榜上的 XX，+8% 出来，爽！谁还在那抠基本面？
<!-- next_speaker: conservative / reason: 想听保守怎么反驳 -->

**保守选手：** 激进你这次运气好。我今天零交易，港股 ZZ 的 PE 还没到我的低吸位 ...
<!-- next_speaker: neutral / reason: 想听中性怎么评判 -->

**中性选手：** 两位风格极端。我 50% 仓位布局了 YY ...
<!-- next_speaker: aggressive / reason: 点名激进回应我的数据 -->
...
- mock 三位 agent 的 `join-discussion` 输出（含 `next_speaker` 注释），验证：
  - 文件 header、三人 PnL、每段发言末尾注释格式正确
  - 编排器按 `next_speaker` 正确传话
  - 连续 2 位不同 agent 投 `end` → 正确结束
  - 总轮数达 50 → 强制结束 + neutral 收官
  - 抢麦护栏：同一 agent 连续 3 次被点名后切换
  - next_speaker 指向自己 / 缺失注释 → 编排器按 fallback 规则处理
  - 某位 agent 调用失败 → 文件出现"缺席"提示并继续
