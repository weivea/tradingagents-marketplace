...
server.tool(
  "get_stock_data",
  "Retrieve OHLCV stock price data for a given ticker symbol and date range",
  {
...
server.tool(
  "get_indicators",
  "Compute a technical indicator for a ticker. Supported: close_50_sma, close_200_sma, close_10_ema, macd, macds, macdh, rsi, boll, boll_ub, boll_lb, atr, vwma",
  {
...
server.tool(
  "get_fundamentals",
  "Get comprehensive company fundamentals: profile, financial data, key statistics",
  {
...
server.tool(
  "get_balance_sheet",
  "Get balance sheet data for a company (quarterly or annual)",
  {
    ticker: z.string().describe("Ticker symbol"),
...
server.tool(
  "get_cashflow",
  "Get cash flow statement for a company (quarterly or annual)",
  {
    ticker: z.string().describe("Ticker symbol"),
    freq: z.enum(["quarterly", "annual"]).optional().default("quarterly").describe("Frequency"),
    curr_date: z.string().optional().describe("Reference date (optional)"),
...
server.tool(
  ...
  "Get income statement for a company (quarterly or annual)",
  {
    ticker: z.string().describe("Ticker symbol"),
    freq: z.enum(["quarterly", "annual"]).optional().default("quarterly").describe("Frequency"),
    curr_date: z.string().optional().describe("Reference date (optional)"),
...
server.tool(
  "get_news",
  "Get company-specific news for a ticker within a date range",
  {
    ticker: z.string().describe("Ticker symbol"),
    start_date: z.string().describe("Start date in yyyy-mm-dd format"),
    end_date: z.string().describe("End date in yyyy-mm-dd format"),
...
server.tool(
  "get_global_news",
  "Get global macroeconomic and market news",
  {
...
server.tool(
  ...
  "Get insider trading transactions and holdings for a company",
  {
    ticker: z.string().describe("Ticker symbol"),
  },
  async (params) => ({
